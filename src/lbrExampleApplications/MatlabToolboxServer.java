 ...
 * the server of this application listens on the port 30005.
 * 
 * */
...
public class MatlabToolboxServer extends RoboticsAPIApplication
{
    ...
    public void run()
    {
    	...
    	while(terminateFlag==false)
        {
    		try
    		{
    			...
    			double[] tempx1={352.3,3.2,103.7,0,0,0};
	        ...
	        while(terminateFlag==false)
	        {
	        	...
	        	else if(daCommand.startsWith("preciseHandGuiding1"))
	        	{
	        	...
	        	else if(daCommand.startsWith("preciseHandGuiding2"))
	        	{
	        		...
	        		try
	        		{
	        			...
	        			getWightAndCoordiantesOfCOMofTool
	        			(daCommand);
	        			tempstring=PreciseHandGuidingForUpload2.LBRiiwa14R820;
	        			// separate data of tool to weight+COM
	        			weightOfTool=toolData[0];
	        			for(int kj=0;kj<3;kj++)
	        			{
	...
	private void attachTheToolToFlange(String string,double[] x) {
		...
		double[] CENTER_OF_MASS_IN_MILLIMETER = { 20.74, -17.59, 60.12 }; 
